...
function blade_grve_print_bg_video_container( $item ) {
	...
	$bg_video_device = blade_grve_array_value( $item, 'video_device', 'no' );
	$bg_image_id = blade_grve_array_value( $item, 'bg_image_id' );
...
?>
	<div id="grve-feature-section" <?php echo implode( ' ', $feature_data ); ?>>
		<div class="grve-map grve-wrapper clearfix" <?php echo blade_grve_get_feature_height( $feature_settings ); ?> data-lat="<?php echo esc_attr( $map_lat ); ?>" data-lng="<?php echo esc_attr( $map_lng ); ?>" data-zoom="<?php echo esc_attr( $map_zoom ); ?>" data-disable-style="<?php echo esc_attr( $map_disable_style ); ?>"><?php echo apply_filters( 'blade_grve_privacy_gmap_fallback', '', $map_lat, $map_lng ); ?></div>
		<?php
