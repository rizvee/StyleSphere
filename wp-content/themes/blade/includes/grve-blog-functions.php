 ...
 * Prints excerpt
 */
if ( !function_exists('blade_grve_print_post_excerpt') ) {
...
if ( !function_exists('blade_grve_get_loop_title_heading_tag') ) {
...
if ( !function_exists('blade_grve_get_loop_title_heading') ) {
...
if ( !function_exists('blade_grve_loop_post_title') ) {
	function blade_grve_loop_post_title( $class = "grve-post-title" ) {
		...
		the_title( '<' . tag_escape( $title_tag ) . ' class="' . esc_attr( $class ). 'grve-' . esc_attr( $title_class ) . '" itemprop="headline">', '</' . tag_escape( $title_tag ) . '>' );
	}
}

if ( !function_exists('blade_grve_loop_post_title_link') ) {
	function blade_grve_loop_post_title_link() {
		$title_tag = blade_grve_get_loop_title_heading_tag();
		$title_class = blade_grve_get_loop_title_heading();
		the_title( '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><' . tag_escape( $title_tag ) . ' class="grve-title grve-text-hover-primary-1 grve-' . esc_attr( $title_class ) . '" itemprop="headline">', '</' . tag_escape( $title_tag ) . '></a>' );
	}
}

if ( !function_exists('blade_grve_loop_post_title_hidden') ) {
 ...
 * Gets Blog Class
 */
if ( !function_exists('blade_grve_get_blog_class') ) {
 ...
 * Gets post class
 */
if ( !function_exists('blade_grve_get_post_class') ) {
 ...
 */
if ( !function_exists('blade_grve_print_blog_data') ) {
	function blade_grve_print_blog_data() {
		...
		switch( $blog_style ) {
			...
			case 'masonry':
				echo 'data-columns="' . esc_attr( $columns ) . '" data-columns-tablet-landscape="' . esc_attr( $columns_tablet_landscape ) . '" data-columns-tablet-portrait="' . esc_attr( $columns_tablet_portrait ) . '" data-columns-mobile="' . esc_attr( $columns_mobile ) . '" data-layout="masonry" data-spinner="' . esc_attr( $item_spinner ) . '" data-gutter-size="' . esc_attr( $gutter_size ) . '"';
				break;
			case 'grid':
				echo 'data-columns="' . esc_attr( $columns ) . '" data-columns-tablet-landscape="' . esc_attr( $columns_tablet_landscape ) . '" data-columns-tablet-portrait="' . esc_attr( $columns_tablet_portrait ) . '" data-columns-mobile="' . esc_attr( $columns_mobile ) . '" data-layout="fitRows" data-spinner="' . esc_attr( $item_spinner ) . '" data-gutter-size="' . esc_attr( $gutter_size ) . '"';
				break;
			default:
				break;
		}
 ...
 */
if ( !function_exists('blade_grve_print_post_feature_media') ) {
 ...
 */
if ( !function_exists('blade_grve_print_post_feature_image') ) {
	function blade_grve_print_post_feature_image() {

		$blog_style = blade_grve_option( 'blog_style', 'large' );
		$blog_image_mode = blade_grve_option( 'blog_image_mode', 'auto' );
		$blog_masonry_image_mode = blade_grve_option( 'blog_masonry_image_mode', 'large' );
 ...
 */
if ( !function_exists('blade_grve_print_post_author_by') ) {
...
?>
				<span><?php esc_html_e( 'By:', 'blade' ) . ' '; ?></span><span><?php the_author_posts_link(); ?></span>
<?php
			if( 'list' == $mode ) {
				echo '</li>';
			} else {
 ...
 */
if ( !function_exists('blade_grve_print_like_counter_overview') ) {
 ...
 * Prints like counter
 */
if ( !function_exists('blade_grve_print_like_counter') ) {
 ...
 * Prints post date
 */
if ( !function_exists('blade_grve_print_post_date') ) {
...
if ( !function_exists('blade_grve_print_post_loop_comments') ) {
 ...
 */
if ( !function_exists('blade_grve_print_post_date_meta') ) {
	function blade_grve_print_post_date_meta( $fallback = '') {
		...
		if ( !empty( $fallback ) ) {
			if ( !blade_grve_visibility( 'blog_date_visibility' ) ) {
?>
			<meta content="<?php echo mysql2date( 'c', $post->post_date ); ?>"/>
<?php
			}
		} else {
?>
 ...
 */
if ( !function_exists('blade_grve_print_post_author') ) {
 ...
 */
if ( !function_exists('blade_grve_print_post_image_meta') ) {
 ...
 */
if ( !function_exists('blade_grve_print_post_audio') ) {
 ...
 */
if ( !function_exists('blade_grve_print_post_video') ) {
 ...
 */
if ( !function_exists('blade_grve_print_blog_meta_bar') ) {
 ...
 */
if ( !function_exists('blade_grve_print_related_posts') ) {
 ...
 */
if ( !function_exists('blade_grve_print_loop_related') ) {
			...
			<article id="grve-related-post-<?php the_ID(); ?>" <?php post_class( 'grve-related-item' ); ?>>
				...
				<a href="<?php echo esc_url( $grve_link ); ?>" target="<?php echo esc_attr( $grve_target ); ?>">
					<div class="grve-content">
						<h5 class="grve-title" itemprop="headline"><?php the_title(); ?></h5>
 ...
 */
if ( !function_exists('blade_grve_likes_callback') ) {
	function blade_grve_likes_callback( $post_id ) {

		check_ajax_referer( 'blade-grve-likes', '_grve_nonce' );
...
if ( !function_exists('blade_grve_likes') ) {
...
if ( !function_exists('blade_grve_print_post_social') ) {
	function blade_grve_print_post_social() {
		...
		if ( !empty( $post_socials ) ) {
			...
			$post_tumblr = blade_grve_option( 'post_social', '', 'tumblr' );
			$post_likes = blade_grve_option( 'post_social', '', 'grve-likes' );
...
if ( !function_exists( 'blade_grve_print_post_structured_data' ) ) {
	function blade_grve_print_post_structured_data( $args = array() ) {

		$empty_image_url = get_template_directory_uri() . '/images/empty/thumbnail.jpg';
		if ( has_post_thumbnail() ) {
		...
		$author_logo_url = blade_grve_option( 'logo', '', 'url' );
		$author_logo_url = str_replace( array( 'http:', 'https:' ), '', $author_logo_url );
	?>
		<span class="grve-hidden">
			...
			<?php if ( blade_grve_visibility( 'blog_author_visibility', '1' ) ) { ?>
			...
			<span class="grve-structured-data" itemprop="publisher" itemscope itemtype="http://schema.org/Organization">
				...
				<span itemprop='logo' itemscope itemtype='http://schema.org/ImageObject'>
					<span itemprop='url'><?php echo esc_url( $author_logo_url ); ?></span>
				</span>
			</span>
			<?php } ?>
