...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			// Load class strings.
			$this->strings = array(
				'page_title'                      => __( 'Theme Plugins', 'blade' ),
				'menu_title'                      => __( 'Theme Plugins', 'blade' ),
				...
				'return'                          => __( 'Return to Theme Plugins Installer', 'blade' ),
		...
		public function install_plugins_page() {
			...
			<div class="tgmpa wrap">
				...
				<?php do_action( 'blade_grve_before_tgmpa_plugins' ); ?>
			...
			foreach ( $plugins[ $this->view_context ] as $slug => $plugin ) {
				...
				if ( $plugin['required'] ) {
					if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
						$table_data[ $i ]['upgrade_notice'] = $plugin['name'] . ' ' . esc_html__( 'is a required plugin and needs to be installed.', 'blade' );
						add_action( "tgmpa_after_plugin_row_{$slug}", array( $this, 'blade_grve_plugin_update_row' ), 10, 2 );
					} elseif( $this->tgmpa->does_plugin_require_update( $slug ) ) {
						$table_data[ $i ]['upgrade_notice'] = $plugin['name'] . ' ' . esc_html__( 'is a required plugin and needs to be updated.', 'blade' );
						add_action( "tgmpa_after_plugin_row_{$slug}", array( $this, 'blade_grve_plugin_update_row' ), 10, 2 );
					}else if( ! $this->tgmpa->is_plugin_active( $slug ) ) {
						$table_data[ $i ]['upgrade_notice'] = $plugin['name'] . ' ' . esc_html__( 'is a required plugin and needs to be activated.', 'blade' );
						add_action( "tgmpa_after_plugin_row_{$slug}", array( $this, 'blade_grve_plugin_update_row' ), 10, 2 );
					}
				}
...
/* 				} */
		...
		public function blade_grve_plugin_update_row( $slug, $item ) {
