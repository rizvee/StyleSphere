...
$action = ( 'network' === $this->parent->args['database'] && $this->parent->args['network_admin'] && is_network_admin() ? './edit.php?action=redux_' . $this->parent->args['opt_name'] : './options.php' );
// Last tab?
...
?>
        <div
            class="grve-redux-container redux-container<?php echo esc_attr( $expanded ); ?>">
            <form
                  method="post" 
				...
				?>
				<?php if ( ! empty( $options_class->no_panel ) ) { ?>
					<input
						type="hidden"
						name="<?php echo esc_attr( $this->parent->args['opt_name'] ); ?>[redux-no_panel]"
						value="<?php echo esc_attr( implode( '|', $options_class->no_panel ) ); ?>"/>
				<?php } ?>
