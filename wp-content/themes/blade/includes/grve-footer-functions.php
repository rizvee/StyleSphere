	...
	?>
		...
		<?php
					...
					foreach ( $footer_sidebars as $footer_sidebar ) {
						echo '<div class="grve-column grve-column-' . $footer_sidebar['column'] . ' grve-tablet-column-' . $footer_sidebar['tablet-column'] . '">';
										...
										foreach ( $social_options as $key => $value ) {
											if ( isset( $options[$key] ) && 1 == $options[$key] && $value ) {
												if ( 'skype' == $key ) {
													...
													echo '<li><a href="' . esc_url( $value ) . '" target="_blank" rel="noopener noreferrer">' . $blade_grve_social_list[$key] . '</a></li>';
												}
											}
										}
										echo '</ul>';
